...
class Usuarios extends Controller{
    ...
    public function validar()
    {
        ...
        if (empty($usuario) || empty($clave)) {
            ...
            if ($data) {
                ...
                $msg = array('msg' => 'Usuario e senha incorretas', 'icono' => 'warning');
            }
        }
        ...
        if (empty($usuario) || empty($nombre)) {
            ...
            if ($id == "") {
                if (!empty($clave) && !empty($confirmar)) {
                    if ($clave != $confirmar) {
                        $msg = array('msg' => 'Senha obrigatórioa', 'icono' => 'warning');
                    } else {
    ...
    public function eliminar(int $id)
    {
        ...
        if ($data == 1) {
            $msg = array('msg' => 'Usuario excluido', 'icono' => 'success');
        }else{
            $msg = array('msg' => 'Erro ao excluir', 'icono' => 'error');
        }
        echo json_encode($msg, JSON_UNESCAPED_UNICODE);
        die();
    }
    ...
    public function cambiarPas()
    {
        if ($_POST) {
            ...
            if (hash("SHA256", $clave) == $user['clave']) {
                ...
                $msg = array('msg' => 'Senha atual incorreta', 'icono' => 'warning');
            }
            echo json_encode($msg, JSON_UNESCAPED_UNICODE);
            die();
        }
