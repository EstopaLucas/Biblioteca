...
class Prestamos extends Controller
{
        ...
        if (empty($libro) || empty($estudiante) || empty($cantidad) || empty($fecha_prestamo) || empty($fecha_devolucion)) {
            ...
            if ($verificar_cant['cantidad'] >= $cantidad) {
                ...
                if ($data > 0) {
                    ...
                    $msg = array('msg' => 'já está emprestado', 'icono' => 'warning');
                } else {
                    $msg = array('msg' => 'Erro ao emprestar', 'icono' => 'error');
                }
            }else{
                $msg = array('msg' => 'Sem estoque', 'icono' => 'warning');
            }
        }
    ...
    public function entregar($id)
    {
        ...
        if ($datos == "ok") {
            $msg = array('msg' => 'Livro devolvido', 'icono' => 'success');
        }else{
            $msg = array('msg' => 'Erro ao devolver livro', 'icono' => 'error');
        }
        echo json_encode($msg, JSON_UNESCAPED_UNICODE);
        die();
